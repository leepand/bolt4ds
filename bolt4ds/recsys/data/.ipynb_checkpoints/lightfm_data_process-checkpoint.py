...
class Dataset(object):
    ...
    def build_item_features(self, data, normalize=True):
        ...
        return builder.build(data)
    def buid_sparse_item_features(self,df,tag_type="type",tag_val="value",col_name="mid"):
        feat_dlist = [{} for _ in dictreverse(self._item_id_mapping)]
        for row in df.itertuples():
            feat_key = '{}_{}'.format(getattr(row,tag_type), str(getattr(row,tag_val)).lower())
            idx = self._item_id_mapping.get(getattr(row,col_name))
            if idx is not None:
