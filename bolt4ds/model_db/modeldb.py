...
class ModelDB(object):
    ...
    def __init__(self,dbpath=''):
        project_name = "algolinkModelDB"
        project_remark = "model logs of trainning"
        nick_name="algolinkDB"
        if dbpath:
            db_file=Path(os.path.join(dbpath, 'model_log.db'))
        else:
            db_file=Path(os.path.join(current_path, 'model_log.db'))
...
if __name__ == '__main__':
    ...
    model_log = ModelDB('test项目', 'test备注')
    '''
    # 训练参数
    tf_param = {}
