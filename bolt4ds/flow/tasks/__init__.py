...
from bolt4ds.flow import targets
from bolt4ds.flow import settings
from  bolt4ds.flow.cache import data as cache
from bolt4ds.flow import cache as bolt4ds_cache
...
class TaskData(luigi.Task):
    ...
    """
    target_class = targets.DataTarget
    target_ext = 'ext'
    persist = ['data']
    ...
    def complete(self, cascade=True):
        ...
        if settings.check_dependencies and cascade and not getattr(self, 'external', False):
    ...
    def output(self):
        ...
        if hasattr(self, 'pipename'):
            ...
            dirpath = bolt4ds.flow.pipes.get_dirpath(self.pipename)
        else:
    ...
    def get_pipename(self):
        ...
        """
        return getattr(self, 'pipename', bolt4ds_cache.pipe_default_name)
    def get_pipe(self):
        """
        Get associated pipe object
        """
        import bolt4ds.flow.pipes
        return bolt4ds.flow.pipes.get_pipe(self.get_pipename())
...
class TaskCache(TaskData):
    ...
    """
    target_class = targets.CacheTarget
    target_ext = 'cache'
...
class TaskCachePandas(TaskData):
    ...
    """
    target_class = targets.PdCacheTarget
    target_ext = 'cache'
...
class TaskJson(TaskData):
    ...
    """
    target_class = targets.JsonTarget
    target_ext = 'json'
...
class TaskPickle(TaskData):
    ...
    """
    target_class = targets.PickleTarget
    target_ext = 'pkl'
...
class TaskCSVPandas(TaskData):
    ...
    """
    target_class = targets.CSVPandasTarget
    target_ext = 'csv'
...
class TaskCSVGZPandas(TaskData):
    ...
    Task which saves to CSV
    """
    target_class = targets.CSVGZPandasTarget
    target_ext = 'csv.gz'
...
class TaskExcelPandas(TaskData):
    ...
    Task which saves to CSV
    """
    target_class = targets.ExcelPandasTarget
    target_ext = 'xlsx'
...
class TaskPqPandas(TaskData):
    ...
    """
    target_class = targets.PqPandasTarget
    target_ext = 'parquet'
...
class TaskAggregator(luigi.Task):
    ...
    example::
        class TaskCollector(bolt4ds.flow.tasks.TaskAggregator):
            def run(self):
                yield Task1()
                yield Task2()
    """
...
class TaskMatplotlib(TaskData):
    ...
    """
    target_class = targets.MatplotlibTarget
    target_ext = 'png'