...
import bolt4ds.ingest.collect as bolt4dscollect
bolt4dscollect.submit = False

from bolt4ds.flow.utils import traverse,print_tree
from bolt4ds.flow.cache import data as data


from bolt4ds.flow.settings import (dirpath,isinitpipe,
                                   isinit,log_level)
from bolt4ds.flow.settings import dir as bolt_dir

print('Welcome to bolt4ds.flow!')# We hope you find it useful. If you run into any problems please raise an issue on github at https://github.com/d6t/d6tflow')

def set_dir(dir=None):
    """
    Initialize bolt4ds.flow
    Args:
        dir (str): data output directory
    """
    if isinitpipe:
        raise RuntimeError('Using d6tpipe, set dir via bolt4ds.flow.pipes.set_default()')
    if dir is None:
        dirpath = dirpath
...
def preview(tasks, indent='', last=True, clip_params=False):
    ...
    if not isinstance(tasks, (list,)):
        tasks = [tasks]
    for t in tasks:
        print(print_tree(t, indent=indent, last=last, clip_params=clip_params))
...
def run(tasks, forced=None, forced_all=False, forced_all_upstream=False, confirm=True, workers=1, abort=True, **kwargs):
    ...
    if forced is not None:
        ...
        for tf in forced:
            for tup in tasks:
                invalidate.append(bolt4ds.flow.taskflow_downstream(tf,tup))
    ...
    opts = {**{'workers':workers, 'local_scheduler':True, 'log_level':log_level},**kwargs}
